...
Посмотреть `MrsMagicBytes` в исходниках новой версии:

```bash
curl -sSL "https://cdn.jsdelivr.net/gh/MetaCubeX/mihomo@<версия>/rules/provider/mrs_reader.go" | grep -n MrsMagicBytes
```

Если версия формата изменилась (не `{'M','R','S',1}`) — **не обновлять**. Файлы нового формата не
прочтут клиенты со старым ядром. Сообщить пользователю и не двигаться дальше без его решения.
